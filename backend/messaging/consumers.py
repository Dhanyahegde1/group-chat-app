...
class ChatConsumer(AsyncWebsocketConsumer):
    ...
    async def connect(self):
        ...
        await self.channel_layer.group_send(
            self.room_group_name,
            {"type": "messages_read", "username": self.scope["url_route"]["kwargs"].get("username", "")}
        )
        await self.channel_layer.group_send(
            self.room_group_name,
            {"type": "user_online", "username": getattr(self, 'username', '')}
        )
    ...
    async def disconnect(self, close_code):
        username = getattr(self, 'username', self.room_name)
        await self.channel_layer.group_send(
             self.room_group_name,
             {"type": "user_offline", "username": self.room_name}
        )
